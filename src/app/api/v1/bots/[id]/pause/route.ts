...
import { getBot, isAdminUser, logBotExecution, pauseBot } from '@/lib/database';
...
export async function POST(
  ...
  try {
    ...
    const updated = await pauseBot(botId);
    if (!updated) return NextResponse.json({ error: 'Bot not found' }, { status: 404 });

    await logBotExecution(botId, 'pause', { actor: user.id });
    return NextResponse.json({ bot: updated });
  } catch (error) {
    console.error('POST /bots/[id]/pause error:', error);
