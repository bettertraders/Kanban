...
import { getBot, isAdminUser, logBotExecution, startBot } from '@/lib/database';
...
export async function POST(
  ...
  try {
    ...
    const updated = await startBot(botId);
    if (!updated) return NextResponse.json({ error: 'Bot not found' }, { status: 404 });

    await logBotExecution(botId, 'start', { actor: user.id });
    return NextResponse.json({ bot: updated });
  } catch (error) {
    console.error('POST /bots/[id]/start error:', error);
