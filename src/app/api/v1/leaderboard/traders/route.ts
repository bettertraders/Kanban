  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        b.id as board_id,
        COUNT(*) FILTER (WHERE t.column_name IN ('Wins','Losses')) as total_trades,
        COUNT(*) FILTER (WHERE t.column_name = 'Wins') as wins,
        COUNT(*) FILTER (WHERE t.column_name = 'Losses') as losses,
        COUNT(*) FILTER (WHERE t.column_name = 'Active') as open_trades,
        COALESCE(SUM(t.pnl_dollar) FILTER (WHERE t.column_name IN ('Wins','Losses')), 0) as total_pnl,
        COALESCE(SUM(t.position_size) FILTER (WHERE t.column_name IN ('Wins','Losses')), 0) as total_volume,
        MAX(t.pnl_dollar) FILTER (WHERE t.column_name IN ('Wins','Losses')) as best_trade,
        MIN(t.pnl_dollar) FILTER (WHERE t.column_name IN ('Wins','Losses')) as worst_trade,
